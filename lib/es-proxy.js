...
exports.configureESProxy = function(app, esHost, esPort, esUser, esPassword) {
  app.use("/__es", function(request, response, next) {
    ...
    function prepend(element, index, array){
      if(element.split('-')[0] === request.session.client){
        array[index] = ''+element;
      }
    }
    var urlCompents = request.url.split('/');
    var indecies = urlCompents[1].split(',');
    if(urlCompents.length >2 ){
      indecies.forEach(prepend);
      urlCompents[1] = indecies.join();
      request.url = urlCompents.join('/');
    }
